...
for p in sorted(ROOT.rglob('*')):
    ...
    rows.append({'path':rel,'sha256':h(p),'size_bytes':p.stat().st_size,'role':role,'canonical_or_generated':'canonical' if role=='canonical' else 'generated_or_evidence','superseded':'no'})
